...
function buildReceiptEmailLink(inv: Invoice): string {
  ...
  const svc = inv.service_name || 'your service'
  const plan = inv.service_plan ? ` (${inv.service_plan})` : ''
  const subject = encodeURIComponent(`Payment Receipt — ${inv.inv_number}`)
  const body = encodeURIComponent(
    [
      ...
      `Invoice:    ${inv.inv_number}`,
      `Service:    ${svc}${plan}`,
      `Amount:     ${formatAmount(inv.total, currency)}`,
      `Paid on:    ${paidDate}`,
      ``,
    ...
    >
      ...
      {isPaid && !readonly && (
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <a
            href={buildReceiptEmailLink(invoice)}
            title={invoice.client_email ? 'Send receipt via email' : 'No client email on file'}
            className={`p-1.5 rounded-md text-blue-500 hover:bg-blue-50 transition-colors ${!invoice.client_email ? 'opacity-40 pointer-events-none' : ''}`}
          >
            <EmailIcon />
          </a>
        </div>
